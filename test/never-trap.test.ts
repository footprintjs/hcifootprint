/**
 * The never-trap invariant, commit half: "a journey whose first step cannot
 * materialise is never committed to." commitJourney() gains exactly one typed
 ...
 * had no ENTRY_NOT_MATERIALIZED reason and no journeyId field.
 */
...
function graphDef(): NavigationGraphDef {
  return {
    pages: {
      ...
      cart: { route: '/cart', actions: { 'place-order': { does: 'Place the order', writes: ['orders'] } } },
    },
    actions: {
...
describe('ENTRY_NOT_MATERIALIZED — the fifth typed refusal', () => {
  it('an agent commit with an unbound entry is refused; the frame never opens; nothing touches state', () => {
    ...
    const refused = session.commitJourney('purchase', { source: 'agent' });
    ...
    expect(session.journeyFrame()).toBeNull(); // the trap room was never entered
  ...
  it('exactly ONE gap row per refusal — fire-rejected, naming the entry step, the journey and the gesture kind', () => {
    const session = buildNavigationGraph('shop', graphDef()).createSession({ node: 'catalog' });
    session.commitJourney('purchase', { source: 'agent' });
  ...
  it('the default principal is agent — a bare commitJourney() gates identically', () => {
    const session = buildNavigationGraph('shop', graphDef()).createSession({ node: 'catalog' });
    expect(session.commitJourney('purchase')).toMatchObject({ ok: false, reason: 'ENTRY_NOT_MATERIALIZED' });
  });
  ...
  it('order preserved: the existing four refusals still answer FIRST', () => {
    ...
    expect(gated.commitJourney('purchase')).toMatchObject({ ok: false, reason: 'PRECONDITION_FAILED' });
...
describe('who passes the gate — every arm that can actually act', () => {
  ...
  it('a navigate-materialisable entry commits fine — the SAME widened handlerFor question', () => {
    ...
    expect(session.commitJourney('to-cart', { source: 'agent' })).toMatchObject({ ok: true });
  });
  ...
  it("a 'user' commit never gates — a human drives the app itself", () => {
    const session = buildNavigationGraph('shop', graphDef()).createSession({ node: 'catalog' });
    expect(session.commitJourney('purchase', { source: 'user' })).toMatchObject({ ok: true });
  });

  it('a registered-but-DISABLED entry commits fine — TOOL_DISABLED is retriable, not missing wiring', () => {
    const session = buildNavigationGraph('shop', graphDef()).createSession({ node: 'catalog' });
    const handle = session.registerActions('catalog', {
...
describe("the gate asks 'could the entry act AT ALL' — instance-keyed wiring counts", () => {
  // A repeats container: one parameterized action, wired per card under
  ...
  it('an entry wired ONLY per instance commits fine — the fire that follows carries the instance key', () => {
    ...
    // journey ENTRY_NOT_MATERIALIZED — a factually false refusal.
    ...
    expect(session.commitJourney('cancel', { source: 'agent' })).toMatchObject({ ok: true });
  });
  ...
  it('a repeats entry with NO wiring at all is still refused — the gate is widened, not weakened', () => {
    ...
    expect(session.commitJourney('cancel', { source: 'agent' })).toMatchObject({
      ok: false,
      reason: 'ENTRY_NOT_MATERIALIZED',
  ...
  it("a SIBLING action's instance keys never open a different entry's gate — the scan is per-affordance", () => {
    const session = buildNavigationGraph('shop', ordersDef()).createSession({
      node: 'orders',
      state: { orderIds: ['o-123'] },
    });
    ...
    expect(session.commitJourney('track', { source: 'agent' })).toMatchObject({
      ok: false,
      reason: 'ENTRY_NOT_MATERIALIZED',
...
describe('the SERVE gate stays structural', () => {
  it('page actions remain reachable while a frame is open (escape guaranteed) — unchanged', () => {
    ...
    expect(served).toContain('shop.leave-journey'); // the frame that opens can always be left
  });
});
