...
describe('mcpServer — a real MCP server backed by a live session', () => {
  it('tools/list returns the FIXED tool set (one per journey + the three generics)', async () => {
    ...
    const purchase = tools.find((t) => t.name === 'shop.journey.purchase')!;
  ...
  it('tools/call routes to the session — open a journey, get readySteps back as data', async () => {
    ...
    session.registerActions('catalog', { handlers: { search: () => undefined } });
    const client = await connectClient(session);
    const res = await client.callTool({ name: 'shop.journey.browse', arguments: {} });
  ...
  it('the tool array is byte-identical across calls (cache-stable, no list_changed)', async () => {
    ...
    await client.callTool({ name: 'shop.journey.browse', arguments: {} });
  ...
  it('a high-effect step returns needs-confirm — the portable, host-agnostic HITL signal', async () => {
    ...
    const client = await connectClient(session);
    await client.callTool({ name: 'shop.journey.purchase', arguments: {} });
    const res = await client.callTool({ name: 'shop.journey.purchase', arguments: { step: 'place-order' } });
  ...
  it('an unbound agent fire surfaces NOT_MATERIALIZED over the server too', async () => {
    ...
    // journey state (the never-trap SERVE stance), while a JOURNEY whose entry is
    ...
    const committed = text(await client.callTool({ name: 'shop.journey.browse', arguments: {} }));
    expect(committed).toMatchObject({ ok: false, reason: 'ENTRY_NOT_MATERIALIZED' });
    expect(session.journeyFrame()).toBeNull();
  });

  it('produced data (a handler return) travels IN the tool result over MCP', async () => {
    const map = buildNavigationGraph('shop', {
      pages: { catalog: { actions: { search: { does: 'Search', writes: ['n'] } } } },
      journeys: { browse: { does: 'Browse', steps: ['search'] } },
    });
    ...
    const client = await connectClient(session);
    await client.callTool({ name: 'shop.journey.browse', arguments: {} });
    const res = await client.callTool({ name: 'shop.journey.browse', arguments: { step: 'search' } });
    const payload = text(res);
  ...
  it('an unknown tool comes back as isError, never a crash', async () => {
    const client = await connectClient(shopSession());
    const res = await client.callTool({ name: 'shop.journey.ghost', arguments: {} });
...
describe('a value JSON cannot encode never costs the caller the whole answer', () => {
  it('a bigint in the handler’s own data crosses as digits instead of throwing', async () => {
    ...
    const map = buildNavigationGraph('shop', {
      pages: { catalog: { actions: { total: { does: 'Read the balance' } } } },
    });
...
describe('settleWithinMs — the settled truth folded into the fire result', () => {
  ...
  function slowShop(delayMs: number, fail?: string) {
    const map = buildNavigationGraph('shop', {
      pages: { catalog: { actions: { search: { does: 'Search dresses', writes: ['n'] } } } },
      journeys: { browse: { does: 'Look around', steps: ['search'] } },
    });
...
describe('a tools/call the transport itself cannot complete', () => {
  it('accepts a call that sends no arguments at all', async () => {
    // `whats_here` takes nothing, so a host that omits `arguments` entirely is
    // the ordinary case — not a missing field to refuse over.
    const client = await connectClient(shopSession());
    const res = text(await client.callTool({ name: 'shop.whats_here' }));
    expect(res['ok']).toBe(true);
    expect(String(res['brief'])).toContain('catalog');
  });

  it('reports a result it cannot serialize as a failed call that NAMES the tool', async () => {
    // An app whose declared input schema refers back to itself. The graph
    // accepts it (it is the app's own object, and the library does not police
    // its shape), but it cannot cross a JSON transport. What must not happen is
    // the throw escaping into the connection: the host gets an error result for
    // the one call, and the session is still there for the next one.
    const selfReferential: Record<string, unknown> = { type: 'object', properties: {} };
    selfReferential['self'] = selfReferential;
    const map = buildNavigationGraph('loop', {
      pages: { home: { actions: { search: { does: 'Search', input: selfReferential, writes: ['n'] } } } },
    });
    const session = map.createSession({ node: 'home', state: { n: 0 }, onWarn: () => undefined });
    ...
    const res = (await client.callTool({ name: 'loop.whats_here' })) as {
      isError?: boolean;
      content: { text: string }[];
    };
    expect(res.isError).toBe(true);
    expect(res.content[0].text).toContain("tool 'loop.whats_here' failed");
    // The connection survived it — the next call still answers.
    const { tools } = await client.listTools();
    expect(tools.map((t) => t.name)).toContain('loop.do_action');
  });
});
