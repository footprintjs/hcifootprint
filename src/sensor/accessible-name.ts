 ...
 * the sensor would rather tell you it did not recognise a control than sign a
 ...
 * same label, and normalising only one side reproduces the exact class of
 * near-miss this is here to remove (the same reasoning route-match.ts gives for
 * reading routes and paths through one `segmentsOf`).
 *
...
export function computeAccessibleName(element: SensorElement, document?: SensorDocument): string {
  ...
  // and that gate is load-bearing rather than an optimisation. HTML calls
  ...
  if (isInputButton(element)) {
    // THE ONE `element.value` READ IN THE WHOLE SUBPATH, and it reads a LABEL:
    // `<input type="submit" value="Save">` is named 'Save'. Pinned by
    // test/sensor-boundary.test.ts so the value-scraping bug class cannot grow
    // back here quietly.
