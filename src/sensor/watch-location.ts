 ...
 * library cannot place has exactly one safe answer, and route-match.ts:16-22
 * already wrote it down —
 *
 *     session.sync(matchRoute(graph.spec.pages, location.pathname) ?? location.pathname);
 *
 * "the caller hands sync() the raw path, which records it off-graph exactly as
 * it does today". That is what happens here. `sync()` records the hop with
 * `unverifiedEdge: true` (atom/types.ts:438-442) — the cursor moved without
 * passing a guard, and the ledger says so instead of inferring an authorized
 * edge nobody fired.
 *
 * THIS IS WHY IT IS OPT-IN (`watchLocation: true`), not on by default. Page ids
 * are author-chosen NAMES, not paths (from-routes.ts:54), so in most apps the raw
 ...
 * actually announces. A `history.pushState()` fires no event, and the sensor
 * does not monkey-patch History to manufacture one: reaching into a host global
 * to overwrite its methods is exactly the reach this library forbids itself, and
 * an app that calls pushState is already the one calling its own router.
 */
