...
state rail, and the one served await — is [Waiting for the app](doc:waiting-for-the-app).
...
[`requireHumanApproval`](doc:receipts#requirehumanapproval--make-approve-enforceable) to refuse a
...
`APPROVAL_DECLINED` — see [Receipts](doc:receipts).
