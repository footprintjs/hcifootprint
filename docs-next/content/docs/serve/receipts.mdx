...
- **A payload that answers differently each time it is read.** The ask binds to a **copy**, so
  changing your object after the yes is refused. The *fire's* payload is not copied — the gate reads
  it, and your handler reads it again a moment later, from the same object. A plain value cannot
  change in between; a **getter** can. An in-process caller that hands `fire()` an object whose
  property returns the approved value to the gate and another value to the handler is inside the
  bullet above, not outside it: it is your own code, on your own side of the channel. Nothing
  crossing a JSON boundary — the Mode B port, the MCP server — can do it. If you relay a live object
  straight from your app into `fire()`, hand it a plain snapshot.
