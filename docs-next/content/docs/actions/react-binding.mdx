...
- **No handler registration.** `registerActions` is already the library's one mount door and
  it needs no framework — call it from your own effect. It stays out of `useControl` because
  the edge id a declaration needs is the *engine's* to resolve, and neither `registerAction` nor
  `registerActions` hands the resolved id back. A binding that rebuilt it from `node` and
